...
export default class Task {
    ...
    create() {
        ...
        container.className = `bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 cursor-grab active:cursor-grabbing group task ${this.options.customClass}`;
        ...
        container.innerHTML = `
            <div class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    <button class="w-6 h-6 rounded-full border-2 border-gray-300 dark:border-gray-600 hover:border-green-500 dark:hover:border-green-400 transition-colors flex items-center justify-center ${this.options.isActive ? '' : 'bg-green-500 dark:bg-green-400 border-green-500 dark:border-green-400'}"
                    ...
                    <div class="flex flex-col">
                        <h3 class="font-medium text-gray-900 dark:text-gray-100 ${this.options.isActive ? '' : 'line-through text-gray-400 dark:text-gray-500'}" title="${this.options.title}">
                            ${this.options.title.length > 30 ? this.options.title.substring(0, 30) + '...' : this.options.title}
                        </h3>
                        ${this.options.description ? `
                            <p class="text-sm text-gray-500 dark:text-gray-400 ${this.options.isActive ? '' : 'line-through'}">
                ...
                <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button class="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors duration-300" title="Editar">
                        <i class="fas fa-pencil text-gray-500 dark:text-gray-400 text-sm"></i>
                    </button>
                    <button class="p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full transition-colors duration-300" title="Excluir">
